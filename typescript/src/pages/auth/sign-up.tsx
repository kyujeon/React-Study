...
function SignUp() {
  ...
  // 필수 동의항목 상태값
  const [serviceAgreed, setServiceAgreed] = useState<boolean>(true); // 서비스 이용약관 동의
  const [privacyAgreed, setPrivacyAgreed] = useState<boolean>(true); // 개인정보 수집 동의
  const [marketingAgreed, setMarketingAgreed] = useState<boolean>(true); // 마케팅 및 광고 수집 동의
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      // user 와 session 두 값 모두 null 이 아닐경우에만 회원가입이 완료되었음을 의미
      if (user && session) {
        toast.success("회원가입을 완료하였습니다.");
        navigate("/sign-in"); // > 로그인 페이지
      }
    } catch (error) {
      console.log(error);
      throw error;
